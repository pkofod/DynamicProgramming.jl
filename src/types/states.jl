...
DiscreteState(X::AbstractVector, F::AbstractVector) = DiscreteState(nothing, X, F, length(X))
...
CommonState(X, F) = CommonState(nothing, X, [F,], length(X)) # should be fixed!
CommonState(name, X, F) = CommonState(name, X, [F,], length(X)) # should be fixed!
...
function Base.display(S::DiscreteStates)
    ...
    @printf "  * %s %s" S.names[1] length(S.X[1])
end

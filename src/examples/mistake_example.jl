...
plot(Xval, V.βEV./U.β, labels = ["EV(1)" "EV(2)"], ylims= (-3664,-3650), xlabel = "mileage", ylabel= "EV")
savefig("/home/pkm/Dropbox/phd/Projekter/Structural Econometrics/right.pdf")

F_wrong = [F1, [ones(Int64, nX) zeros(nX, nX-1)]]
S_wrong = State(Xval, F_wrong)
V_wrong = IntegratedValueFunction(nX, 2)
solve(U, S_wrong, V_wrong, Poly())
plot(Xval, V_wrong.βEV./U.β, labels = ["EV(1)" "EV(2)"], ylims= (-3576,-3562), xlabel = "mileage", ylabel= "EV")

...
Poly() = Poly(1e-10, 100, 1e-10, 5, 1e-10, 10, 1e-10, 10, 0.5)
# should have
# immutable Poly
# vfi
# policy
# end
function solve(U, S; verbose = false)
    V = IntegratedValueFunction(S.nX, length(U.U))
    ret = solve(U, S, V, VFI(); verbose = verbose)
    return V, ret
end
...
function solve(U, S, V, method::Poly; verbose = false)
    ...
    for i = 1:method.maximum_poly
        i_vfi += solve(U, S, V, vfi; verbose = verbose)
